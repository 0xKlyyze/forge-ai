        ...
        >
            <Link to={`/project/${project.id}`}>
                <motion.div
                    ...
                    className="group relative h-[240px] bg-zinc-900/40 hover:bg-zinc-900/60 border border-white/5 hover:border-blue-500/30 rounded-2xl p-6 transition-all duration-300 flex flex-col justify-between shadow-xl"
                >
                    ...
                    <div className="relative z-10" style={{ transform: "translateZ(20px)" }}>
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                {project.icon ? (
                                    <div className="h-12 w-12 rounded-xl bg-black/50 flex items-center justify-center border border-white/10 overflow-hidden shadow-lg">
                                        <img src={project.icon} alt={project.name} className="w-full h-full object-cover" />
                                    </div>
                                ) : (
                                    <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-zinc-800 to-zinc-950 flex items-center justify-center border border-white/10 shadow-lg">
                                        <Code className="h-6 w-6 text-zinc-500 group-hover:text-white transition-colors" />
                                    </div>
                                )}
                                <div className="min-w-0">
                                    <h3 className="font-bold text-base text-zinc-100 group-hover:text-white transition-colors truncate max-w-[120px]" title={project.name}>
                                        {project.name}
                                    </h3>
                                    <div className="flex items-center gap-2 text-[10px] text-zinc-500 mt-1">
                                        <span className={`h-1.5 w-1.5 rounded-full ${project.status === 'planning' ? 'bg-yellow-500/80' : project.status === 'building' ? 'bg-blue-500/80' : 'bg-green-500/80'}`} />
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 text-white/5 hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all scale-90 group-hover:scale-100"
                                onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    onDelete(project);
                    ...
                    <div className="relative z-10 mt-auto space-y-3" style={{ transform: "translateZ(10px)" }}>
                        <div className="flex items-center justify-between text-[10px] text-zinc-500 font-mono bg-black/40 p-2.5 rounded-xl border border-white/5 backdrop-blur-sm">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span>{project.is_owner ? 'Private' : 'Shared'}</span>
                            </div>
                        </div>
                    </div>
                </motion.div>
