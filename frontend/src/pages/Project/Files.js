...
export default function ProjectFiles() {
  ...
  const [activeTab, setActiveTab] = useState('grid'); // 'grid' or 'editor'
    ...
    <div className="h-full flex flex-col bg-background/50">
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
             <Tabs value={activeTab} onValueChange={setActiveTab} className="w-[400px]">
                <TabsList>
                    <TabsTrigger value="grid">Browse Artifacts</TabsTrigger>
                                ...
                                <Card key={file.id} className="group bg-secondary/10 border-white/5 hover:border-primary/50 transition-all cursor-pointer overflow-hidden relative">
                                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent>
                                                <DropdownMenuItem onClick={() => { setActiveTab('editor'); navigate(`/project/${projectId}/files`); }}>
                            ...
                            {filteredFiles.map(file => (
                                <div key={file.id} className="flex items-center justify-between p-3 rounded bg-secondary/10 border border-white/5 hover:bg-secondary/20 transition-colors group">
                                    ...
                                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <Button variant="secondary" size="sm" onClick={() => { setActiveTab('editor'); }}>Open Editor</Button>
                ...
                <Dialog open={!!previewFile} onOpenChange={() => setPreviewFile(null)}>
                    <DialogContent className="max-w-3xl h-[80vh] flex flex-col">
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <Button onClick={() => { setPreviewFile(null); setActiveTab('editor'); }}>Open Full Editor</Button>
                        </div>
                    </DialogContent>
                </Dialog>
            </div>
